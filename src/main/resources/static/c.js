      ...
      self.LoadProfilesByLocationId = function () {
            ...
            $.ajax({
                ...
                url: '/product',
                    ...
                    console.log("profiles: " + ko.toJSON(self.ProfileList()));
            }).fail(error => {
...
/*        // this is the response todo
        ...
        // Update the Current Profile with the Id in case this was an "Insert".
        //self.Profile.Id(response.Id);
        var profile = ko.unwrap(self.Profile);
        profile.Id = response.Id;
        //console.log("--- profile: " + ko.toJSON(profile));
...
*//*
    ...
    // Filter the Profile Types in the table of Profiles.
    //self.typeFilter = ko.observable('');
    self.filteredProfiles = ko.computed(function () {
        var filter = self.companyFilter().toLowerCase();
        if (!filter || filter === "") {
    ...
    // Select a Profile from the list on the default view.
    self.SelectProfile = function (data) {
		console.log("selected raw    : " + ko.toJSON(data));
        self.companyFilter("");
    ...
    self.UpdateProfiles = function (profile) {
        var recordFound = false;
        ko.utils.arrayFirst(self.ProfileList(), function (item) {
            if (item.Id === profile.Id) {
                console.log("trying to update list...");
                console.log("profile: " + ko.toJSON(profile));
                console.log("item:    " + ko.toJSON(item));
                ko.mapping.fromJS(profile, item);
                recordFound = true;
                return;
            }
        });
...
var ProfilePostViewModel = function (data, locationId) {
    var self = this;
    self.Id = data.id;
